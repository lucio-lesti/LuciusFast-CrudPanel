...
class Datatables
{
   ...
   * @return mixed
   */
  public function join($table, $fk, $type = NULL, $join_condition = NULL)
  {
    $this->joins[] = array($table, $fk, $type, $join_condition);
    $this->ci->db->join($table, $fk, $type, $join_condition);
    return $this;
  }
  /**
  ...
  private function get_ordering()
  {
    ...
    //print_r($this->ci->input->post('order'));
  ...
  {
    //PRINT'<PRE>2';print_r($_POST['searchFilter']);  
    //PRINT'<PRE>3';print_r($this->select);
    ...
    $searchFilterCount = 0;
    foreach ($searchFilter as $kFilter => $vFilter) {
      $AND_OP = "";
      if ($searchFilterCount > 0) {
        $AND_OP = " AND ";
      }
      if (is_array($vFilter['value'])) {
        if (count($vFilter['value']) > 0) {
          $vFilter['field'] = str_replace("[]", "", $vFilter['field']);
          $IN_CONDITION = implode(',', array_map('Datatables::add_quotes', $vFilter['value'])) ;
          $CHECK_IN_CONDITION = str_replace("'","",$IN_CONDITION);
          if(trim($CHECK_IN_CONDITION) != ""){
            $sWhere .= "$AND_OP" . $this->select[$vFilter['field']] . " IN(" . $IN_CONDITION. ")";
          }

        }
      } else {
        if($vFilter['value'] != ""){
          $sWhere .= "$AND_OP" .$this->select[$vFilter['field']] . " LIKE '%" . $vFilter['value'] . "%' ";
        }
  ...
  private function get_total_results($filtering = FALSE)
  {
    ...
    foreach ($this->where as $val)
      $this->ci->db->where($val[0], $val[1], $val[2], $val[3]);
    foreach ($this->or_where as $val)
      $this->ci->db->or_where($val[0], $val[1], $val[2], $val[3]);
...
/* Generated by Harviacode Codeigniter CRUD Generator 2021-06-04 17:19:19 */
/* http://harviacode.com */