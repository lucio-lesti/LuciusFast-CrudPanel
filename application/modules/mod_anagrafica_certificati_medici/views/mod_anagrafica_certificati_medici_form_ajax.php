   ...
   <div <?php if ($winForm == 'TRUE') {
   					...
   					<form action="<?php echo $action; ?>" method="post" name="<?php echo $frm_module_name . '_' . $id; ?>" id="<?php echo $frm_module_name . '_' . $id; ?>" <?php $preventDefault; ?>\>
   						...
   						<div class="col-md-4">
   							<div class="form-group">
   								...
   								<SELECT name='tipologia' id='tipologia' class="form-control" style="width:100%;padding: 6px 12px;font-size:14px;
									border-top-right-radius:0px;border-bottom-right-radius:0px;
									border-top-left-radius:0px;border-bottom-left-radius:0px;
									border:1px solid #ccc">
   									<OPTION <?php echo ($tipologia == 'AGONISTICO' ? 'SELECTED' : ''); ?> VALUE='AGONISTICO'>AGONISTICO</OPTION>
   									<OPTION <?php echo ($tipologia == 'NON AGONISTICO' ? 'SELECTED' : ''); ?> VALUE='NON AGONISTICO'>NON AGONISTICO</OPTION>
   								</SELECT>
   							</div>
   						</div>
   						<div class="col-md-4">
   							<div class="form-group">
   								...
   								<div class="input-group">
   									...
   									<input type="text" class="form-control datemask" name="data_certificato" id="data_certificato" placeholder="Data Certificato" autocomplete="off" style="background-color:#FFFFFF" value="<?php echo $data_certificato; ?>" />
   								</div>
   							</div>
   						</div>
   						<div class="col-md-4">
   							<div class="form-group">
								   ...
								   <a style="cursor:pointer" onclick="aggiornaDataUnMese()">
								   <i class="fa fa-hourglass-half" style="padding: 2px 4px;border:1px solid #ccc;"></i>
								</a> 								
								   <div class="input-group">
   									<div class="input-group-addon"><i class="fa fa-calendar"></i></div>
   									<input type="text" class="form-control datemask" name="data_scadenza" id="data_scadenza" placeholder="Data Scadenza Certificato" autocomplete="off" style="background-color:#FFFFFF" value="<?php echo $data_scadenza; ?>" />
   								</div>
   							</div>
   						</div>
   						...
   						<div class="col-md-4">
   							<div class="form-group">
   								<label for="documento_upload">Documento Upload <?php echo form_error('documento_upload') ?></label>
   								<div class="input-group">
   							...
   							<?php
								if ($documento_upload != '') {
									...
									<a href='" . base_url() . "mod_anagrafica_certificati_medici/scaricaAllegatoBlob/_mod_anagrafica_certificati_medici/documento_upload/" . $id . "' target='_blank'>Scarica Allegato</a>";
										echo "<br><a style='cursor:pointer' onclick=\"rimuoviAllegatoBlob('mod_anagrafica_certificati_medici', 'documento_upload','$id','FIELD_BLOB')\"><img src=\"".base_url()."/assets/images/delete2.png\" width='32'/> Elimina allegato</a></div>";
									} else {
   						...
   						<div class="col-md-12">
   							<div class="form-group">
   								<label for="note">Altri Allegati <?php echo form_error("allegati") ?>
   								...
   								<ul>
   									<?php
										...
										foreach ($allegati as $key => $allegato) {
											...
											if (($id != '')) {
												echo "<li id='allegato_" . $count . "'><a href='" . base_url() . "mod_anagrafica_certificati_medici/scaricaAllegato/mod_anagrafica_certificati_medici/" . $id . "/" . $nomeAllegato . "' target='_blank'>" . $allegato['allegato'] . "</a>";
												echo "<a style='cursor:pointer' onclick=\"rimuoviAllegato('mod_anagrafica_certificati_medici', '$id','$nomeAllegato','allegato_$count')\"> | " . $icon_delete . "</a></li>";
											} else {
   	...
   	<?php
		if ($type_action == 'create') {
			$ajaxAction = 'mod_anagrafica_certificati_medici/create_action';
		} else {
			$ajaxAction = 'mod_anagrafica_certificati_medici/update_action';
		}
		...
		?>
   	<?php echo $this->load->view("jsconfig/mod_anagrafica_certificati_medici_form_config.js.php", $data, true); ?>
   	
	<script>
		function aggiornaDataUnMese(){
			showBiscuit();
			$.ajax({
   				url: baseURL + "/mod_anagrafica_certificati_medici/aggiornaDataUnMese/",
   				type: 'post',
   				dataType: 'json',
				data:{
					data_certificato:document.getElementById('data_certificato').value
				},   
   				success: function(jsonData) {
					document.getElementById('data_scadenza').value = jsonData.data_scadenza_unmese;
   					hideBiscuit();
