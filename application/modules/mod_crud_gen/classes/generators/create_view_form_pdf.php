...
class create_view_form_pdf extends AbstractGenerator{
	...
	public static function output($param_gen){
		...
		switch($form_columns_nr_layout){
			...
			case 2:
			case '2':
				$column_field_begin ="<div class=\"col-md-6\" style=\"float: left; width: 50%;padding-bottom:5px\">";
			...
			case 3:
			case '3':
				$column_field_begin ="<div class=\"col-md-4\"  style=\"float: left; width: 32%;padding-bottom:5px\">";
				$column_field_end ="</div>";	
				$nrColsRowAllowed = 3;	
				$widthInput = "85%";
			break;	
		...
		foreach ($non_pk as $row) {
			...
			if(in_array($row['COLUMN_NAME'],$field_name)){		
				...
				switch($row["DATA_TYPE"]){
					...
					case 'longblob':
						$string .= "\n
								<label for=\"".$row["DATA_TYPE"]."\">".$helper->label($etichettaLabel)."</label>";
						$string .=" 
							<?php 
								if($".$row["COLUMN_NAME"]." != ''){
					...
					case 'text':	
						$string .= "\n\t
								<label for=\"".$row["COLUMN_NAME"]."\">".$helper->label($etichettaLabel)." </label>
			...
			$nrRow++;
			if($countRowPrint == $nrColsRowAllowed){
				$string .= "<p style=\"clear: left;\" /></p>";
				$countRowPrint = 1;
			} else {
				$countRowPrint++;
			}
