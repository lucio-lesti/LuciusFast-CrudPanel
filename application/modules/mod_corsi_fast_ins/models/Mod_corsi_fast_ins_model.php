...
class Mod_corsi_fast_ins_model extends BaseModel
{
	...
	public function json(Array $searchFilter){
		...
		$fieldsInfoDetails["fk_affiliazione"]['table'] = "mod_corsi";
		$fieldsInfoDetails["fk_affiliazione"]['field_name'] = "fk_affiliazione";
	...
	**/
	...
	public function copyCourses($idAffiliaz, $lista_corsi){		
		...
		foreach($row as $k => $v){
			...
			$sql ="SELECT count(*)as count_cors
				...
				WHERE mod_corsi.nome='".$v['nome']."' AND mod_esercizi.id = $idEserc";
			*/
			$sql ="SELECT count(*)as count_cors
				FROM mod_corsi 				
		...
		//$res = $this->db->insert("mod_corsi", $data);
	...
	public function insertPagamentiDaCorsi(Array $request = NULL, $fk_anagrafica = null, $fk_corso = null, $data_iscrizione = null){
		...
		$data['fk_causale_pagamento']  = 6; //PAGAMENTO ALLIEVO MENSILE
