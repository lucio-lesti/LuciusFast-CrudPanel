   ...
   <div <?php if ($winForm == 'TRUE') {
   	...
   	<div class="col-md-8">
   		<h3>
   			...
   			<a>
   				<u> Corsi <?php if ($_SESSION['fast_ins_corsi'] == 'Y') {
									echo " - Inserimento Veloce";
								} ?></u>
   			</a>
   			<?php
				if ((isset($id)) && ($id != "")) {
   			...
   			<div class="box box-primary">
   				...
   				<div class="box-body">
   					...
   					<form action="<?php echo $action; ?>" method="post" name="<?php echo $frm_module_name . '_' . $id; ?>" id="<?php echo $frm_module_name . '_' . $id; ?>" <?php $preventDefault; ?>\>
   						...
   						<div class="col-md-4">
   							<div class="form-group">
   								<label for="varchar">Dicitura Corso per stampe<?php echo form_error('dicitura_corso') ?></label>
   								<div class="input-group">
   									<div class="input-group-addon"><i class="fa fa-text-height"></i></div>
   									<input type="text" <?php echo $disabled; ?> class="form-control" maxlength='255' name="dicitura_corso" id="dicitura_corso" placeholder="Dicitura Corso per stampe" autocomplete="off" value="<?php echo $dicitura_corso; ?>" />
   								</div>
   							</div>
   						</div>
   							...
   							<div class="col-md-4">
   								<div class="form-group">
									...
									border:1px solid #ccc" name='fk_affiliazione_datalist_inp' id='fk_affiliazione_datalist_inp' value='<?php echo $fk_affiliazione_label; ?>'></span>
   									...
   									<datalist name='fk_affiliazione_datalist' id='fk_affiliazione_datalist'>
   										...
   										<?php
											if (isset($master_details_list)) {
												...
												foreach ($master_details_list as  $key => $master_details) {
													...
													echo '<div class="tab-pane ' . $active . '" id="' .  $master_details['id'] . '" role="tabpanel" aria-labelledby="' . $key . '-tab">';
   	...
   	<script>
   		...
   		arrayValidationFields['winMasterDetailMulti_mod_corsi_insegnanti'] = [];
   		arrayValidationFields['winMasterDetailMulti_mod_corsi_insegnanti']['fk_anagrafica'] = [];
   		arrayValidationFields['winMasterDetailMulti_mod_corsi_insegnanti']['fk_anagrafica']['field_type'] = "int"
   		...
   		js_custom_operations_list['winMasterDetail_mod_anagrafica_corsi'].check_data_iscrizione = function() {
   			var ret = true;

   			$.ajax({
   				url: baseURL + "/mod_corsi/checkDataIscrizione",
   				type: 'post',
   				dataType: 'json',
   				async: false,
   				data: {
   					data_iscrizione: document.getElementById('data_iscrizione').value,
   					fk_affiliazione: document.getElementById('fk_affiliazione').value
   				},
   				success: function(jsonData) {
   					console.log(jsonData);
   					if (jsonData.ret == 'ko') {
   						document.getElementById('msg_err').style.display = "block";
   						document.getElementById('msg_err').innerHTML = "Data Iscrizione non valida, deve essere compresa nelle date dell'esercizio.";
   						ev.preventDefault();
   						ret = false;
   					}
   				}
   		...
   		js_custom_operations_list['winMasterDetail_mod_anagrafica_corsi'].check_ins_allievo = function() {
   			var ret = true;
   			if (document.getElementById('fk_anagrafica').value.trim() == "") {
   				document.getElementById('msg_err').style.display = "block";
   				document.getElementById('msg_err').innerHTML = "Allievo obbligatorio";
   				ev.preventDefault();
   				ret = false;
   			}
   		...
   		js_custom_operations_list['winMasterDetail_mod_corsi_giorni_orari'].check_hour = function() {
   			var ret = validateHhMm(document.getElementById('ora_da').value);
   			if (ret === false) {
   				document.getElementById('msg_err').style.display = "block";
   				document.getElementById('msg_err').innerHTML = "Ora Da non valida";
   				ev.preventDefault();
   			}

   			if (ret === true) {
   				var ret = validateHhMm(document.getElementById('ora_a').value);
   				if (ret === false) {
   					document.getElementById('msg_err').style.display = "block";
   					document.getElementById('msg_err').innerHTML = "Ora a non valida";
   					ev.preventDefault();
   				}
   			}

   			if (parseInt(document.getElementById('ora_da').value) > parseInt(document.getElementById('ora_a').value)) {
   				document.getElementById('msg_err').style.display = "block";
   				document.getElementById('msg_err').innerHTML = "Ora Da non puo essere superiore a Ora A";
   				ev.preventDefault();
   				ret = false;
   			}
