...
class Login_model extends CI_Model
{
    ...
    function loginMe($email, $password)
    {
        ...
        $this->db->join('core_roles as Roles','Roles.roleid = BaseTbl.roleid');
    ...
    {
        $result = $this->db->insert('core_reset_password', $data);
    ...
    {
        $this->db->select('id');
        $this->db->from('core_reset_password');
        $this->db->where('email', $email);
    ...
    function createPasswordUser($email, $password)
    {
        $this->db->where('email', $email);
        $this->db->where('isdeleted', 0);
        $this->db->update('core_users', array('password'=>getHashedPassword($password)));
        $this->db->delete('core_reset_password', array('email'=>$email));
    }
    ...
    {
        $this->db->trans_start();
        $this->db->insert('core_log', $logInfo);
    ...
    function lastLoginInfo($userid)
    {
        ...
        $this->db->limit(1);
        $query = $this->db->get('core_log as BaseTbl');
